		...
		<div class="usercoverimg">
		 {{$user->printUserCoverImage()}}
			<div class="userMaininfo">
                ...
                <div class="title">
                    <h3><span>{{$user->getName()}}</span>
                        @if((bool)$user->is_immediate_available)
                            <span class="avail">{{__('Immediate Available For Work')}}</span>
                        @endif
                    </h3>
                    <div class="desi"><span><i class="fa fa-map-marker" aria-hidden="true"></i>{{$user->getLocation()}}</span></div>
                    <div class="membersinc"><span><i class="fa fa-history" aria-hidden="true"></i> {{__('Member Since')}}, {{$user->created_at->format('F d, Y')}}</span></div>
                </div>    
            </div>
